...
from .SplAtConv2d import SplAtConv2d

__all__ = ['MobileNetV2','mobilenet_v3_small','mobilenet_v3_large']
...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio):
        ...
        layers.extend([
            ...
            # pw-linear
            SplAtConv2d(hidden_dim, oup, 1, 1, 0, bias=False),
            nn.BatchNorm2d(oup),
        ])
...
class MobileNetV3_Large(nn.Module):
    def __init__(self, ):
        ...
        self.bneck = nn.Sequential(
            ...
            Block(3, 80, 184, 80, hswish(), None, 1),
            Block(3, 80, 480, 112, hswish(), SeModule(112), 2),
...
class MobileNetV3_Small(nn.Module):
    def __init__(self, ):
        ...
        self.bneck = nn.Sequential(
            ...
            Block(5, 48, 144, 48, hswish(), SeModule(48), 2),
...
def mobilenet_v3_small(pretrained,**kwargs):
    ...
    if pretrained:
        pretrained_model = torch.load('./pre_model/mbv3_small.old.pth.tar')['state_dict']
        state = model.state_dict()
        for key in state.keys():
            if 'module.'+key in pretrained_model.keys():
...
def mobilenet_v3_large(pretrained,**kwargs):
    ...
    if pretrained:
        pretrained_model = torch.load('./pre_model/mbv3_large.old.pth.tar')['state_dict']
        state = model.state_dict()
        for key in state.keys():
            if 'module.'+key in pretrained_model.keys():
                state[key] = pretrained_model['module.'+key]
